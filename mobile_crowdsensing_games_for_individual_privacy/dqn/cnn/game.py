...
def game_2user(n_user=N_USER_SINGLE, func=FUNC):
    ...
    for episode in tqdm(range(MAX_EPISODE)):
        ...
        for step in range(MAX_STEP):
            ...
            cur_user1_state = np.array([pre_user1_action, pre_mcs_action])
            cur_user2_state = np.array([pre_user2_action, pre_mcs_action])
            ...
            cur_user1_action_index = model_user1.select_action(cur_user1_state)
            cur_user1_action = agent_user1.get_action_value(cur_user1_action_index)

            cur_user2_action_index = model_user2.select_action(cur_user2_state)
            ...
            next_user1_state = np.array([cur_mcs_action_index, cur_user1_action_index])
            next_user2_state = np.array([cur_mcs_action_index, cur_user2_action_index])
            # 5. optimize model
            model_mcs.optimize_model(cur_mcs_state, next_mcs_state, cur_mcs_action_index, cur_r_mcs, batch_size=BATCH_SIZE)
            model_user1.optimize_model(cur_user1_state, next_user1_state, cur_user1_action_index, cur_r_user1, batch_size=BATCH_SIZE)
            model_user2.optimize_model(cur_user2_state, next_user2_state, cur_user2_action_index, cur_r_user2, batch_size=BATCH_SIZE)
...
def game_n_user(n_user=N_USER_MULTI, func=FUNC):
    agent_mcs = MCSAgent(MCS_ACTION)
